...
void Token::print() {
  if (const auto doublePtr(std::get_if<double>(&this->value)); doublePtr) {
    ...
    std::cout << TokenTypeToString(this->token) << " "
              << std::get<std::string>(this->value) << "val\n";
  }
}
...
void MathParser::next_token() {
  ...
  while (current_t.token == TokenType::None) {
    ...
    if (std::isalpha(static_cast<unsigned char>(*iter)) || *iter == '_') {
      ...
      while (std::isalpha(static_cast<unsigned char>(*iter)) || *iter == '_' ||
      ...
      switch (*this->iter) {
      case '+':
        current_t.token = TokenType::Operator;
        current_t.value = "+";
        break;
      case '-':
        current_t.token = TokenType::Operator;
...
Exprs MathParser::list() {
  Exprs list_e = expr();
  // Check for null/error?
  if (!list_e.value.has_value()) {
    std::cout << static_cast<int>(list_e.value.error())
              << " - ERROR WHILE PARSING";
  }
  while (current_t.token == TokenType::Comma) {
    next_token();
